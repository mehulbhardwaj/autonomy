...
class DummyTM:
    def get_next_task(self, assignee=None, team=None, explain=False):
        return ({"number": 1, "title": "t", "labels": [], "created_at": "2025-01-01T00:00:00Z"}, {})
...
def test_cli_startup_time():
    start = time.time()
    subprocess.run([sys.executable, "-m", "src.cli.main", "--help"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
    assert time.time() - start < 2.5

@pytest.mark.usefixtures("tmp_path")
def test_next_command_performance(monkeypatch):
    monkeypatch.setattr("src.tasks.task_manager.TaskManager", lambda *a, **kw: DummyTM())
